...
CREATE TABLE `menu` (
	...
	`is_delete`	BOOLEAN	NOT NULL	DEFAULT FALSE,
	`is_visible`	BOOLEAN	NOT NULL	DEFAULT TRUE	COMMENT 'FALSE를 하고 보조메뉴에만 보이게 설정 가능하다.'
);
...
CREATE TABLE `anniversary_coupon_vendor` (
	...
	`anniversary _code`	CHAR(2)	NOT NULL,
...
CREATE TABLE `anniversary_type` (
	`code`	CHAR(2)	NOT NULL	COMMENT '01 생일 고정',
	`name`	VARCHAR(20)	NOT NULL	COMMENT '생일'
);
...
DROP TABLE IF EXISTS `CopyOfuser_notification_type`;

CREATE TABLE `CopyOfuser_notification_type` (
	`id`	BIGINT	NOT NULL,
...
CREATE TABLE `delivery_price` (
	`id`	BIGINT	NOT NULL,
	`price`	INT	NOT NULL	COMMENT '0, 4000'
);

DROP TABLE IF EXISTS `reward_point_type`;

CREATE TABLE `reward_point_type` (
	`code`	char(2)	NOT NULL	COMMENT '회원가입, 리뷰, 리뷰+사진',
	`name`	varchar(20)	NOT NULL,
...
ALTER TABLE `CopyOfuser_notification_type` ADD CONSTRAINT `PK_COPYOFUSER_NOTIFICATION_TYPE` PRIMARY KEY (
	`id`
);
...
ALTER TABLE `CopyOfuser_notification_type` ADD CONSTRAINT `FK_user_store_TO_CopyOfuser_notification_type_1` FOREIGN KEY (
	`user_store_id`
)
...
ALTER TABLE `CopyOfuser_notification_type` ADD CONSTRAINT `FK_notification_type_TO_CopyOfuser_notification_type_1` FOREIGN KEY (
	`notification_type_code`
)
REFERENCES `notification_type` (
	`code`
