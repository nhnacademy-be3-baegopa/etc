...
(
    ...
    `user_role_code`      char(2)      NOT NULL DEFAULT 'A1' COMMENT '회원 권한 코드',
...
(
    ...
    `store_role_code` CHAR(2)     NOT NULL PRIMARY KEY COMMENT '매장 역할 코드',
    `name`            VARCHAR(10) NOT NULL COMMENT '이름'
);
...
(
    ...
    `user_id`         BIGINT  NOT NULL COMMENT '회원 식별번호',
    `store_role_code` CHAR(2) NOT NULL COMMENT '역할 코드',
...
(
    ...
    `option_id`          BIGINT       NOT NULL COMMENT '옵션 식별번호',
...
(
    ...
    `count`                  INT UNSIGNED NOT NULL DEFAULT 1 COMMENT '주문수량'
);
...
(
    ...
    `order_id`        BIGINT     NOT NULL COMMENT '주문 식별번호',
    `issue_coupon_id` binary(16) NOT NULL COMMENT '발급쿠폰 식별번호'
);
...
(
    ...
    `user_role_code` char(2)     NOT NULL PRIMARY KEY COMMENT '회원 권한 코드',
    `name`           varchar(20) NOT NULL COMMENT '이름'
);

DROP TABLE IF EXISTS `menu_search`;
CREATE TABLE `menu_search`
(

    `menu_id`     BIGINT       NULL COMMENT '메뉴 식별번호',
    `name`        VARCHAR(100) NULL COMMENT '메뉴 이름',
    `description` TEXT         NULL COMMENT '설명'
) ENGINE=Mroonga DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
ALTER TABLE `user`
    ADD CONSTRAINT FOREIGN KEY (user_role_code) REFERENCES `user_role` (`user_role_code`);
...
ALTER TABLE `user_store`
    ADD CONSTRAINT FOREIGN KEY (user_id) REFERENCES `user` (`user_id`);
ALTER TABLE `user_store`
    ADD CONSTRAINT FOREIGN KEY (store_role_code) REFERENCES `store_role` (`store_role_code`);
ALTER TABLE `user_store`
...
ALTER TABLE `menu_option`
    ADD CONSTRAINT FOREIGN KEY (option_id) REFERENCES `option` (`option_id`);
ALTER TABLE `menu_option`
...
CREATE TRIGGER `menu_insert_trigger`
    AFTER INSERT
    ON `menu`
    FOR EACH ROW insert into menu_search
...
CREATE TRIGGER `menu_update_trigger`
    AFTER UPDATE
    ON `menu`
    FOR EACH ROW update menu_search
                 set name        = new.name,
                     description = new.description
...
ALTER TABLE `order_coupon` add column `store_id` BIGINT NULL COMMENT '매장 식별번호';
ALTER TABLE `order_coupon` ADD CONSTRAINT FOREIGN KEY (store_id) REFERENCES `store` (`store_id`);
